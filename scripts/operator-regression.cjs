...
assert.equal(context.getOperatorDefs().medic.speedBoostMult, undefined);
assert.match(context.getOperatorDefs().medic.skillTextEn, /restore 800 HP/);
assert.match(context.getOperatorDefs().medic.skillTextEn, /double bullet damage/);
...
assert.equal(r.player.armor, 0);
assert.equal(r.player.damageImmunityTimer, 12);
assert.equal(r.player.medicSpeedBoostTimer, 0);
assert.equal(r.player.abilityActiveTimer, 20);
r.player.damageImmunityTimer = 0;
context.updateRaid(0.05);
assert.equal(r.player.damageReductionTimer, 8);
close(r.player.damageReductionMult, 0.5);
assert.equal(r.player.supportFirepowerTimer, 8);
