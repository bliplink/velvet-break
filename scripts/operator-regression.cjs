...
function raid(operatorId = 'engineer') {
  context.state.raid = { player: { operatorId, x: 0, z: 0, yaw: 0, pitch: 0, radius: 0.5, health: 1000, maxHealth: 1000, armor: 200, maxArmor: 200, stamina: 100, maxStamina: 100, skillUses: 4, abilityActiveTimer: 0, utilityItems: 1, ammoInMag: 30, recoilKick: 0 }, enemies: [] };
...
assert.equal(context.getOperatorDefs().medic.abilityDuration, 33);
close(context.getOperatorDefs().medic.speedBoostMult, 1.5);
assert.match(context.getOperatorDefs().medic.skillTextEn, /2\.5x bullet damage/);
...
assert.equal(r.player.armor, 200);
assert.equal(r.player.damageImmunityTimer, 8);
assert.equal(r.player.medicSpeedBoostTimer, 33);
assert.equal(r.player.abilityActiveTimer, 33);
r.player.damageImmunityTimer = 0;
context.updateRaid(0.05);
assert.equal(r.player.damageReductionTimer, 25);
close(r.player.damageReductionMult, 0.3);
assert.equal(r.player.supportFirepowerTimer, 25);
...
r = raid();
r.mouseWorldPointer = { clientX: 400, clientY: 600 };
const slowed = { id: 'slowed', x: 5, z: 0, health: 1000, radius: 0.5 };
r.enemies.push(slowed);
scene.pick = () => ({ hit: true, pickedPoint: new BABYLON.Vector3(5, 0, 0), pickedMesh: { name: 'ground' } });
key('KeyO');
assert.ok(r.stunGrenadeTargeting);
assert.equal(r.player.stunGrenadeItems, 1);
key('KeyO');
assert.equal(r.player.stunGrenadeItems, 0);
assert.ok(r.player.stunGrenadeThrow.held.getChildMeshes().length >= 4);
context.updateRaid(0.45);
assert.ok(r.player.stunGrenadeThrow.projectile);
context.updateRaid(0.6);
assert.equal(r.player.stunGrenadeThrow, null);
assert.ok(slowed.engineerSlowTimer > 9 && slowed.engineerSlowTimer <= 10);
const slowX = slowed.x;
context.moveEntityWithCollision(slowed, 4, 0, 0.5);
close(slowed.x - slowX, 1.8);
context.updateRaid(10);
assert.equal(slowed.engineerSlowTimer, 0);
context.updateRaid(40);
assert.equal(r.player.stunGrenadeItems, 2);
context.clearRaid();
assert.equal(scene.meshes.filter((mesh) => /stun-grenade/.test(mesh.name)).length, 0);

r = raid('medic');
assert.equal(r.player.benjaminKillCount, undefined);
assert.equal(r.incendiaryFields, undefined);
engine.dispose();
console.log('PASS: profiles, Benjamin major buff, recoil, skill cap, stun/recovery, protection, boss utility resistance, barriers, instant executions, kill rewards, incendiary and stun-grenade targeting/throw geometry, 10s slow, resupply, cleanup.');
