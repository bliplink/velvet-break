...
function updateBaseCamera() {
  ...
  camera.fov = lerp(camera.fov, 0.82, 0.08);
...
function syncPlayerCamera() {
  ...
  camera.fov = lerp(camera.fov, 0.82 + nearHitPulse * 0.012 - recoilKick * 0.01, 0.22);
...
function syncPlayerCamera() {
  ...
  let fovTarget = 0.82 - aimBlend * (hasOptic ? 0.15 : 0.08) + nearHitPulse * 0.012 - recoilKick * 0.01;
  if (operator.id === 'assault' && player.abilityActiveTimer > 0) {
    fovTarget += 0.018;
  }
...
function useOperatorAbility() {
  ...
  if (operator.id === 'assault') {
    ...
    notify(
      L(`\u900f\u89c6\u626b\u63cf\u5df2\u542f\u52a8\uff0c20 \u79d2\u5185\u5df2\u63ed\u793a\u672c\u533a\u57df ${revealedCount} \u540d\u654c\u4eba\u4e0e\u5175\u79cd\u3002`, `Tactical Scan activated. ${revealedCount} enemies and classes are revealed in this sector for 20s.`),
      'success',
    );
  }
