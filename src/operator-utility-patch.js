  ...
  const boot = () => {
    ...
    const activateSkill = () => {
      ...
      if (operator.id === 'assault') {
        ...
        notify(L(`战术增益已启动：恢复 800 生命，12 秒免伤；随后 8 秒伤害减半并造成双倍子弹伤害。剩余技能 ${player.skillUses}/4。`, `Combat boost active: +800 HP, 12s immunity, then 8s half damage and double bullet damage. Uses left: ${player.skillUses}/4.`), 'success');
      }
      syncUtilityUi();
      syncHud();
    };
